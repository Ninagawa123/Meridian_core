...
#  TsySKIP, PcSKIP: 連番データの取りこぼし数を表示します.今はちょっと多めです.
#  周波数はTeensyが担っています. Teensy用スクリプトMeridian_core_for_Teensy40.inoの(TS-1-1)で変更できます. 周波数を50Hzまで下げるとゼロに近くなります.
...
#定数
TITLE_VERSION="Meridian Console v22.0416" #DPGのウィンドウタイトル兼バージョン表示
...
frame_clock = 0 #送信するframe_clock(0-199)
    ...
    while (True):
        ...
        with closing(sock):
            while True:
                ...
                #クロックの受信
                frame_clock_resv_past = frame_clock_resv #前回受信したクロックを今回のpastとしてキープ
                frame_clock_resv = r_meridim_disp[88] & 0b0000000011111111 #クロックを受信

                if(frame_clock_resv == frame_clock_resv_past + 1): #受信したクロックが送信したものから+2であればスキップなし
                    temp[0] &= 0b1111111011111111 #PCのスキップフラグを下げる
                elif((frame_clock_resv == 0 ) and (frame_clock_resv_past == 199)):
                    temp[0] &= 0b1111111011111111 #PCのスキップフラグを下げる
                else:
                    print("Found data skipping on PC.")
                    temp[0] |= 0b0000000100000000 #PCのスキップフラグを上げる
                    error_count_pc_skip += 1 #スキップカウントをプラス
                
                #送信用クロックの準備
                frame_clock += 1 #送信用のframe_clockをカウントアップ
                if frame_clock >199:
                    frame_clock=0
                temp[0] &= 0b1111111100000000 #下位8ビットをクリア
                temp[0] += frame_clock #下位8ビットに受信カウントに+1したものを格納
